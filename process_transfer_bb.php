...
{
    $token = "A2q0Wa55Gp3a9CKOdBfCEiljlZ43TfS1EKeMXOHU1Gv"; // ใส่ Token ที่สร้างไว้
    $data = array(
        'message' => $message
    );
...
function check_employee($employee_id) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM employee_back WHERE employee_id = :employee_id");
...
function check_duplicate_transfer($bank_account_no, $amount) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM tranferlog_back_ptop 
    ...
    switch ($action) {
        ...
        case 'transfer':
            ...
            if (isset($result['transfer_status']) && $result['transfer_status'] === 'PROCESSING') {
                ...
                $stmt = $conn->prepare("INSERT INTO tranferlog_back (phonenumber, amount, date_time, employee_name, employee_id) VALUES (:phonenumber, :amount, :date_time, :employee_name, :employee_id)");
                $stmt->execute([
