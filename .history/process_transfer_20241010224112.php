...
function check_employee($employee_id) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM employee_guuci WHERE employee_id = :employee_id");
...
function check_duplicate_transfer($bank_account_no, $amount) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM tranferlog_guuci_ptop 
    ...
    switch ($action) {
        ...
        case 'transfer':
            ...
            if (isset($result['transfer_status']) && $result['transfer_status'] === 'PROCESSING') {
                ...
                $stmt = $conn->prepare("INSERT INTO tranferlog_guuci_ptop (amount, date_time, bankAccountNo) VALUES (:amount, :date_time, :bank_account_no)");
                $stmt->execute([
                    'amount' => $data['amount'],
                    'date_time' => $date_time,
                ...
                $stmt = $conn->prepare("INSERT INTO tranferlog_guuci (phonenumber, amount, date_time, employee_name, employee_id) VALUES (:phonenumber, :amount, :date_time, :employee_name, :employee_id)");
                $stmt->execute([
