...
function sendLineNotifyLog($message) {
    ...
    $token = 'zx9oRLPulJzFoRxa7APCoC0EmxxK2DYOEjDOYc1g731'; // แทนที่ด้วย token ของคุณ
    $headers = [
...
{
    $token = "zx9oRLPulJzFoRxa7APCoC0EmxxK2DYOEjDOYc1g731"; // ใส่ Token ที่สร้างไว้
    $data = array(
        'message' => $message
    );
...
function check_employee($employee_id) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM employee_allslot WHERE employee_id = :employee_id");
...
function check_duplicate_transfer($bank_account_no, $amount) {
    $conn = get_connection();
    $stmt = $conn->prepare("SELECT * FROM tranferlog_allslot_ptop 
    ...
    switch ($action) {
        ...
        case 'transfer':
            ...
            if (isset($result['transfer_status']) && $result['transfer_status'] === 'PROCESSING') {
                ...
                $stmt = $conn->prepare("INSERT INTO tranferlog_allslot_ptop (amount, date_time, bankAccountNo) VALUES (:amount, :date_time, :bank_account_no)");
                $stmt->execute([
                    'amount' => $data['amount'],
                    'date_time' => $date_time,
                ...
                $stmt = $conn->prepare("INSERT INTO tranferlog_allslot (phonenumber, amount, date_time, employee_name, employee_id) VALUES (:phonenumber, :amount, :date_time, :employee_name, :employee_id)");
                $stmt->execute([
